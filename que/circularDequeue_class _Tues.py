...
class CircularDeque(CircularQueueOneSlotEmpty):
    ...
    #일부 연산들은 상속을 사용해 사용가능

    def delete_front(self): # 전단에서 삭제(dequeue)
        return self.dequeue()
    
    def get_front(self): #전단에서 검색(peek와 같음)
        return self.peek()
    
    def add_rear(self, item): # 후단에서 삽입(enqueue)
    ...
    def delete_rear(self): # 후단에서 삭제
        if self.is_empty():
            ...
            item = self.array[self.rear] # rear
